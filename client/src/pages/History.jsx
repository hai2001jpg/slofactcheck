...
const History = () => {
    ...
    return (
        <div className="flex flex-row min-h-screen bg-[#1B1B1B] overflow-x-hidden">
            <Sidebar />
            <div className="flex flex-col items-center min-h-full w-full lg:w-7/8 pt-20 pb-4 lg:py-8 gap-4 lg:gap-8">
                ...
                {loading && <div className="text-white text-xl sm:text-2xl montserrat font-semibold">{t("common:status.loadingData")}</div>}

                {error && <div className="text-red-500 text-xl sm:text-2xl montserrat font-semibold">{error}</div>}

                {analyses.length === 0 && !loading && !error && <div>
                    <div className="text-gray-400 text-xl sm:text-2xl montserrat font-semibold">{t("history:empty")}</div>
                    <div className="flex flex-col items-center w-full gap-8">
                        <button
                            ...
                            onClick={handleStartNewAnalysis}>
                                ...
                                <span className="text-gray-300 montserrat whitespace-nowrap">{t("history:startNewAnalysis")}</span>
                        </button>
                    </div>
                </div>}
